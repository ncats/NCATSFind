...
function getSettings(callback){
	...
	if(EXT_TYPE==FIREFOX_EXT){
		...
		getValue("showTutorial",function(val){
			settings["showTutorial"]=val;
			if(callback)callback(settings);
		});
          ...
          return "<img style='height:"+defSize+"px;"+"width:"+defSize+"px;' src='" + urlFor(text) + "'>";
                    ...
                    Zepto('.ncgchover').on("mouseenter",function () {
                        if(!Zepto(this).hasClass("unii")){
                            ...
                            if(str==undefined){
                                lookup[uniilook]="d";
								var murl = "https://tripod.nih.gov/servlet/resolver/?structure=" + encodeURIComponent(uniilook).replace("+","%2B") + "&force=true";
...
function setValue(key,value){
	...
	switch(EXT_TYPE){
		...
		case FIREFOX_EXT:
			var uid= (Math.round(Math.random()*100000));
			self.postMessage({type:"set",key:key,id:uid,value:value});
			break;
		case GM_EXT:
		//TODO:IMPLEMENT
			break;
...
function display2(str, wx, wy, strtitle){
	...
	$("<div class='mystr'><input style='width: 100%;font-size: smaller;font-family: monospace;' type='textbox' value='" + str+ "'/><img style='cursor:pointer;width: 100%;' src='" + urlFor(str) + "'></div>")
	.dialog({closeText: "hide",title:strtitle ,position: 'top',show: {effect: 'fade', duration: 350},hide: {effect: 'fade', duration: 250}});
	$(".ui-dialog").css('z-index', 99999); 
